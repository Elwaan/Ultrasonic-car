...
{
	DIO_Fun_Set_Pin_Dir(LCD_CONTROL_PORT, LCD_E, OUTPUT);
	DIO_Fun_Set_Pin_Dir(LCD_CONTROL_PORT, LCD_RW, OUTPUT);
	DIO_Fun_Set_Pin_Dir(LCD_CONTROL_PORT, LCD_RS, OUTPUT);
	
	DIO_Fun_Set_Pin_Dir(LCD_PORT, LCD_D4, OUTPUT);
	DIO_Fun_Set_Pin_Dir(LCD_PORT, LCD_D5, OUTPUT);
	DIO_Fun_Set_Pin_Dir(LCD_PORT, LCD_D6, OUTPUT);
	DIO_Fun_Set_Pin_Dir(LCD_PORT, LCD_D7, OUTPUT);
	
	DIO_Fun_Set_Pin_Val(LCD_CONTROL_PORT, LCD_RW, LOW); // always write on lcd
...
{
	DIO_Fun_Set_Pin_Val(LCD_CONTROL_PORT, LCD_RS, HIGH); // data
...
// 	DIO_F_Set_Pin_Val(LCD_PORT, LCD_D4, GET_BIT(data, 4));
	...
	DIO_Fun_Set_Port_Val(LCD_PORT, data & 0xF0);
	LCD_F_Trigger();
	
	DIO_Fun_Set_Port_Val(LCD_PORT, data<<4);
	LCD_F_Trigger();
...
void LCD_F_Init(void)
{
	DIO_Fun_Set_Port_Dir(LCD_PORT, OUTPUT);
	DIO_Fun_Set_Pin_Dir(LCD_CONTROL_PORT, LCD_E, OUTPUT);
	DIO_Fun_Set_Pin_Dir(LCD_CONTROL_PORT, LCD_RW, OUTPUT);
	DIO_Fun_Set_Pin_Dir(LCD_CONTROL_PORT, LCD_RS, OUTPUT);
...
{
	DIO_Fun_Set_Pin_Val(LCD_CONTROL_PORT, LCD_RS, HIGH); // data
	DIO_Fun_Set_Port_Val(LCD_PORT, data);
	LCD_F_Trigger();
}
...
{
	DIO_Fun_Set_Pin_Val(LCD_CONTROL_PORT, LCD_RS, LOW); // command
	DIO_Fun_Set_Port_Val(LCD_PORT, command);
	LCD_F_Trigger();
}
#endif
...
/*void LCD_F_Write_String(U8_t* str)
{
	while(*str != '\0')
	{
...
void LCD_F_Write_String(const U8_t* str){
	int i;
	for(i=0; str[i]!= '\0'; i++)
	LCD_F_Write_Char(str[i]);
}
